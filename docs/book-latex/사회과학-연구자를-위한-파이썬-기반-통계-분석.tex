...
\begin{verbatim}
C:\Users\voidm\AppData\Local\Temp\ipykernel_35100\1340021530.py:4: FutureWarning:
...
\begin{tcolorbox}[enhanced jigsaw, toprule=.15mm, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, left=2mm, colback=white, opacityback=0, bottomtitle=1mm, titlerule=0mm, toptitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{📌 핵심 목표}, arc=.35mm, rightrule=.15mm, leftrule=.75mm, bottomrule=.15mm, breakable]
...
\textbf{2. 가정 검토 (차이 점수의 정규성):} 차이
점수(\texttt{Stress\_Diff})가 정규분포를 따르는지 Shapiro-Wilk 검정으로
확인합니다.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 차이 점수의 정규성 검정 (Shapiro{-}Wilk Test)}
\NormalTok{shapiro\_diff }\OperatorTok{=}\NormalTok{ stats.shapiro(df\_paired[}\StringTok{\textquotesingle{}Stress\_Diff\textquotesingle{}}\NormalTok{])}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialStringTok{Shapiro{-}Wilk Test (Stress Differences): Statistic=}\SpecialCharTok{\{}\NormalTok{shapiro\_diff}\SpecialCharTok{.}\NormalTok{statistic}\SpecialCharTok{:.3f\}}\SpecialStringTok{, p{-}value=}\SpecialCharTok{\{}\NormalTok{shapiro\_diff}\SpecialCharTok{.}\NormalTok{pvalue}\SpecialCharTok{:.3f\}}\SpecialStringTok{"}\NormalTok{)}

\CommentTok{\# 시각적 확인 (히스토그램)}
\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{5}\NormalTok{))}
\NormalTok{sns.histplot(df\_paired[}\StringTok{\textquotesingle{}Stress\_Diff\textquotesingle{}}\NormalTok{], kde}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{\textquotesingle{}Distribution of Stress Score Differences (Post {-} Pre)\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{\textquotesingle{}Stress Difference\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{\textquotesingle{}Frequency\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-group-difference-tests_files/figure-latex/cell-8-output-2.png}}
...
\CommentTok{\# 잔차 정규성 검정 (Shapiro{-}Wilk)}
\NormalTok{shapiro\_resid }\OperatorTok{=}\NormalTok{ stats.shapiro(residuals)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"}\CharTok{\textbackslash{}n}\SpecialStringTok{{-}{-}{-} Normality Test of Residuals (Shapiro{-}Wilk) {-}{-}{-}"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Statistic: }\SpecialCharTok{\{}\NormalTok{shapiro\_resid}\SpecialCharTok{.}\NormalTok{statistic}\SpecialCharTok{:.3f\}}\SpecialStringTok{, p{-}value: }\SpecialCharTok{\{}\NormalTok{shapiro\_resid}\SpecialCharTok{.}\NormalTok{pvalue}\SpecialCharTok{:.3f\}}\SpecialStringTok{"}\NormalTok{)}

\CommentTok{\# 잔차 히스토그램 (시각적 확인)}
\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{5}\NormalTok{))}
\NormalTok{sns.histplot(residuals, kde}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{\textquotesingle{}Histogram of Residuals\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{\textquotesingle{}Residuals\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{\textquotesingle{}Frequency\textquotesingle{}}\NormalTok{)}
\NormalTok{plt.show()}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-group-difference-tests_files/figure-latex/cell-15-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{3a_sub_files/figure-latex/fig-t-distribution-comparison-output-1.png}}

}

\caption{\label{fig-t-distribution-comparison}자유도(df)에 따른 t-분포와
표준정규분포(Z) 비교}

\end{figure}%

위 그래프(Figure~\ref{fig-t-distribution-comparison})에서 볼 수 있듯이,
자유도가 작을수록(df=2) t-분포는 Z-분포보다 훨씬 넓게 퍼져 있으며 꼬리가
두껍습니다. 자유도가 증가함에 따라(df=5, df=30) 점차 Z-분포에 가까워지는
것을 확인할 수 있습니다.

\begin{itemize}
\tightlist
\item
  \textbf{가설 검정에서의 활용:} t-분포의 PDF(및 누적분포함수 CDF)는
  특정 t-통계량 값이 귀무가설 하에서 얼마나 발생 가능성이 있는지를
  확률적으로 계산하는 데 사용됩니다. 즉, p-값은 이 t-분포를 기준으로
  계산됩니다. (예: 양측 검정 p-값 = \(P(|T_{\nu}| \ge |t_{observed}|)\))
\end{itemize}

\chapter{요약}\label{uxc694uxc57d-1}

t-분포는 모집단 표준편차(\(\sigma\))를 모르고 표본 표준편차(\(s\))를
사용하여 통계적 추론을 할 때 사용되는 필수적인 확률 분포입니다. 표본에서
계산된 t-통계량은 (모집단 정규성 가정 하에) 이 분포를 따르며, 분포의
정확한 모양은 분석에 사용된 독립적인 정보의 수인 자유도(df)에 의해
결정됩니다. t-분포는 정규분포보다 두꺼운 꼬리를 가지며(특히 df가 작을
때), 이는 \(\sigma\) 추정에 따른 불확실성을 반영합니다. 자유도가 커지면
정규분포에 수렴합니다.

t-검정의 p-값은 귀무가설 하에서 해당 t-분포를 기준으로 계산되므로,
t-분포에 대한 이해는 t-검정 결과를 올바르게 해석하는 데 중요합니다. 본
교재의 3장 및 관련 장에서는 이 t-분포를 기반으로 하는 다양한 t-검정
기법들을 파이썬을 통해 실제로 적용하고 해석하는 방법을 다룹니다.
